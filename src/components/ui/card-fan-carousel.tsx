...
const FAN_POSITIONS = [
  { rot: -12, scale: 0.73, x: -36, y: 3.0, zIndex: 1 },
  { rot: -8, scale: 0.83, x: -26, y: 1.6, zIndex: 2 },
  { rot: -4, scale: 0.93, x: -13, y: 0.5, zIndex: 3 },
...
function getSlotConfig(totalCards: number, slot: number) {
  ...
  // Spread symmetrically around the midpoint so an even card count doesn't
  // leave the fan lopsided (one more card on the left than the right).
  const mid = (totalCards - 1) / 2
  const distance = mid > 0 ? (slot - mid) / mid : 0
  const absDistance = Math.abs(distance)
  return {
    rot: distance * 12,
    scale: 1.0 - 0.27 * absDistance * absDistance,
    x: distance * 36,
            ...
            const image = (
              <div className="relative h-full w-full overflow-hidden">
                <Image
                  ...
                  fill
                  loading="lazy"
                  // Must track `.fan-card`'s own width per breakpoint in
                  // globals.css exactly — undersizing this serves a
                  // smaller-than-rendered source image that reads as
                  // blurry once the browser upscales it to fill the box.
                  sizes="(min-width: 1024px) 20rem, (min-width: 768px) 17rem, (min-width: 640px) 14rem, (min-width: 480px) 12.5rem, 11rem"
